  ...
  async getBasicMetrics(): Promise<{
    ...
    try {
      ...
      return {
        ...
        activeConnections: Number.parseInt(connectionRow?.active_connections) || 0,
        serverless: true, // Neon Database 是无服务器架构
      }
    }
    catch (error) {
      console.error('[DatabaseManager] Failed to get basic metrics:', error)
      throw new Error(`Failed to retrieve basic metrics: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async getConnectionStatus(): Promise<{
    ...
    try {
      ...
      return {
        ...
        activeConnections: Number.parseInt(connRow?.active_connections) || 0,
