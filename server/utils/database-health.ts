  ...
  try {
    ...
    return {
      ...
      utilization: result[0]?.max_connections
        ? ((totalConnections[0]?.total_connections || 0) / result[0].max_connections * 100).toFixed(2)
        : '0',
    }
  }
  catch (error) {
    throw new Error(`Failed to get connection pool status: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export async function getDatabaseMetrics() {
  try {
    ...
    return {
      ...
      cacheHitRatio: stat.blocks_read && stat.blocks_hit
