...
async function logApiAccess(
  apiKeyId: string, // 改为 string (UUID)
  method: string,
  endpoint: string,
  statusCode: number,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (apiKeyRecord.expiresAt && currentTime > apiKeyRecord.expiresAt) {
...
const OPEN_API_PERMISSION_MAP: Array<{
  ...
  // 示例：
  // { pattern: /^\/api\/open\/users/, read: 'users:read', write: 'users:write' },
]
