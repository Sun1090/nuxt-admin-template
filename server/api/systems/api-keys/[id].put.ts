...
const updateApiKeySchema = z.object({
  name: z.string().min(1, 'API Key名称不能为空').max(100, 'API Key名称不能超过100个字符').optional(),
  remark: z.string().max(500, '描述不能超过500个字符').optional().nullable(),
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // 处理过期时间
    let expiresAt: number | Date | null
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (validatedData.permissions) {
        // 删除现有权限
        await tx.delete(apiKeyPermissions)
    ...
    throw createError({
      statusCode: 500,
      message: `更新API Key失败: ${error.message}`,
    })
  }
})
